...
const scientistSchema = z.object({
  ...
  birth_year: z.number().describe('出生年份，如 1867'),
  death_year: z.number().optional().describe('去世年份，如果还在世则不填'),
  nationality: z.string().describe('国籍'),
  fields: z.array(z.string()).describe('研究领域列表'),
  spouse: z.string().optional().describe('配偶姓名'),
  children_count: z.number().optional().describe('子女数量'),
  children: z
    .array(
      z.object({
        name: z.string().describe('子女姓名'),
        age: z.number().optional().describe('子女年龄'),
        gender: z.string().optional().describe('子女性别'),
      }),
    )
    .optional()
    .describe('子女姓名列表'),
...
try {
  ...
  if (result.spouse) {
    console.log(`💑 伴侣: ${result.spouse}`)
  }
  if (result.children_count) {
    console.log(`👶 子女数量: ${result.children_count}`)
    if (result.children) {
      console.log(`\n👶 子女信息 (${result.children.length}个):`)
      result.children.forEach((child, index) => {
        console.log(`   ${index + 1}. ${child.name}`)
        console.log(`      年龄: ${child.age}`)
        console.log(`      性别: ${child.gender}`)
      })
    }
  }
