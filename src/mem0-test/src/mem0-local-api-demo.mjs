...
const BASE_URL = process.env.MEM0_LOCAL_BASE_URL
const USER_ID = process.env.MEM0_LOCAL_USER_ID
...
class LocalMem0Client {
  ...
  async search(query, { filters, topK, threshold, explain } = {}) {
  ...
  if (action === 'search') {
    log(
      ...
      await client.search('用户的饮食限制是什么？', {
        ...
        topK: Number(process.env.MEM0_LOCAL_TOP_K),
      }),
    )
    return
  }
