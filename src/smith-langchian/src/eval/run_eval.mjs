...
const client = new Client({ apiKey: process.env.LANGCHAIN_SMITH_API_KEY })
/** 被评测的 RAG Agent */
...
async function main() {
  const result = await evaluate(runRagAgent, {
    ...
    client,
    experimentPrefix: `rag-openevals-${process.env.MODEL ?? 'qwen'}`,
    maxConcurrency: 2,
  })
  ...
  const project = process.env.LANGCHAIN_SMITH_PROJECT ?? 'default'
  console.log('✅ 评测完成')
