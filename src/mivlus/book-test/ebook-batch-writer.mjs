...
// 加载环境变量（本脚本直接走阿里云批处理接口，用不到 shared/model.mjs 的客户端）
...
// 阿里云 DashScope 批处理 embedding 相关配置
const DASHSCOPE_BASE_URL =
  process.env.DASHSCOPE_BASE_URL ?? 'https://dashscope.aliyuncs.com/api/v1'
const ASYNC_EMBEDDING_MODEL =
  process.env.EBOOK_ASYNC_EMBEDDING_MODEL ?? 'text-embedding-async-v2'
const BATCH_MAX_WAIT_MS =
  parseInt(process.env.EBOOK_BATCH_MAX_WAIT_MS) || 300000
const BATCH_POLL_INTERVAL_MS =
  parseInt(process.env.EBOOK_BATCH_POLL_INTERVAL_MS) || 5000
// 临时文件统一放 tmp/，不污染仓库根目录
...
async function getUploadPolicy(modelName) {
  const response = await fetch(
    `${DASHSCOPE_BASE_URL}/uploads?action=getPolicy&model=${modelName}`,
    {
      method: 'GET',
      headers: {
...
async function getInsertedChapters(bookId) {
  try {
    ...
    // 提取所有章节号并去重
    const chapterNums = [
      ...new Set(queryResult.data.map(item => item.chapter_num)),
    ]
...
function saveChapterAsTextFile(chapterTexts, chapterNum) {
  ...
  const fileName = join(TMP_DIR, `temp_chapter_${chapterNum}.txt`)
 ...
 * 等待批处理任务完成
 */
async function waitForBatchTask(taskId, maxWaitTime = BATCH_MAX_WAIT_MS) {
...
async function downloadAndExtractBatchResult(resultUrl) {
  mkdirSync(TMP_DIR, { recursive: true })
  const tempGzFile = join(TMP_DIR, 'temp_batch_result.gz')
  const tempResultFile = join(TMP_DIR, 'temp_batch_result.jsonl')
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      if (parsed.code && parsed.code !== 200) {
        console.error(
          `  ❌ 第 ${i + 1} 行返回错误: ${parsed.code} - ${parsed.message}`,
        )
        errorLines.push(i + 1)
        continue
      }
  ...
  if (errorLines.length > 0) {
    console.warn(
      `  ⚠️  共 ${errorLines.length} 行解析失败: ${errorLines.join(', ')}`,
    )
  }
...
async function insertToMilvus(embeddings, chapterNum, chunks) {
  const insertData = embeddings.map((embedding, index) => ({
    id: `${BOOK_ID}_${chapterNum}_${index}`,
    book_id: BOOK_ID,
    book_name: BOOK_NAME,
...
async function main() {
  try {
    ...
    // 获取已插入的章节
    const insertedChapters = await getInsertedChapters(BOOK_ID)
    ...
    for (
      ...
      try {
        ...
        const insertedCount = await insertToMilvus(
          embeddings,
          chapterNum,
          chunks,
        )
        console.log(
          `  ✓ 已插入 ${insertedCount} 条记录（累计: ${totalProcessed + insertedCount}）\n`,
        )
