...
function parseWebSearchBlocks(text: WebSearchToolOutput): WebSearchResultItem[] {
  ...
  for (const block of blocks) {
    ...
    // 摘要可跨行，截至下一个字段标记或块末尾
    const summary =
      block.match(/摘要:\s*([\s\S]*?)(?=\n\s*(?:相关度:|网站名称:|发布时间:|$))/)?.[1]?.trim() ?? ''
    const relatedScore = block.match(/相关度:\s*(.+)/)?.[1]?.trim()
    const publishedAt = block.match(/发布时间:\s*(\S+)/)?.[1]?.trim()
    // 旧格式兼容
    const siteName = block.match(/网站名称:\s*(.+)/)?.[1]?.trim()
    if (title || url || summary || ref) {
      items.push({ ref, title, url, summary, siteName, publishedAt, relatedScore })
    }
  }
  return items
}

/** 从 `web_search` 输出中提取「综合答案」 */
function extractWebSearchAnswer(text: WebSearchToolOutput): string | null {
  const m = text.match(/综合答案:\s*([\s\S]*?)(?=\n\n引用:|$)/)
...
function WebSearchToolPanel({
  ...
  const answer = extractWebSearchAnswer(output)
  ...
  return (
    <div className="tool-panel tool-panel--web-search">
      ...
      {answer ? (
        <div className="tool-panel__answer">{answer}</div>
      ) : null}
      {items.length > 0 ? (
          ...
          {items.map((it, idx) => (
            <li key={`${it.ref}-${idx}`} className="tool-panel__result">
              ...
              <div className="tool-panel__meta">
                {it.relatedScore ? (
                  <span className="tool-panel__score">相关度 {it.relatedScore}</span>
                ) : null}
                {it.siteName ? (
...
/** 解析 `send_mail` 输出结果（成功/失败/邮件 ID） */
type MailOutputParse = {
  status: 'success' | 'error'
  emailId?: string
  message: string
}

function parseMailOutput(output: string): MailOutputParse | null {
  if (!output?.trim()) return null

  const successMatch = output.match(/^邮件发送成功！.*邮件 ID: (\S+)/)
  if (successMatch) {
    return {
      status: 'success',
      emailId: successMatch[1],
      message: '邮件已发送',
    }
  }

  if (/^邮件发送失败：/.test(output)) {
    return { status: 'error', message: output }
  }

  if (/^邮件发送失败，原因是:/.test(output)) {
    const msg = output.replace(/^邮件发送失败，原因是:\s*/, '')
...
function MailOutputDisplay({ output }: { output: string }) {
  const parsed = parseMailOutput(output)
  if (!parsed) {
    return (
  ...
  if (parsed.status === 'error') {
    return (
      <div className="tool-panel__mail-status tool-panel__mail-status--error" role="alert">
        {parsed.message}
      </div>
    )
  }
  return (
    <div className="tool-panel__mail-status tool-panel__mail-status--success" role="status">
      <span className="tool-panel__mail-status-icon">✓</span>
      邮件发送成功
      {parsed.emailId ? (
        <span className="tool-panel__mail-id">ID: {parsed.emailId}</span>
      ) : null}
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      {inProgress ? (
        ...
        <MailOutputDisplay output={output} />
      ) : null}
    </div>
  )
}
