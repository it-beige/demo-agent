...
const MEMORY_TTL = Number(process.env.MEMORY_TTL_SECONDS)
const KEY_PREFIX = process.env.MEMORY_KEY_PREFIX

const USER_ID = process.env.MEM0_USER_ID
const SESSION_ID = process.env.MEM0_SESSION_ID
const MEM0_TOP_K = Number(process.env.MEM0_TOP_K)
...
if (!process.env.MEM0_API_KEY || !process.env.API_KEY) {
  console.error('需要 MEM0_API_KEY 与 API_KEY')
  process.exit(1)
}
...
const classifier = createModel({ temperature: 0 }).withStructuredOutput(
  memorySchema,
)
