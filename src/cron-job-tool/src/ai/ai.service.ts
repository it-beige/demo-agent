...
export class AiService {
  ...
  private readonly maxIterations: number = 30;
...
定时任务类型选择规则（非常重要）：
- 用户说"X 分钟/小时/天后”“到点提醒”（相对时间、一次性）=> 用 \`cron_job\` + \`type=at\` + \`delayMs\`（延迟毫秒数，例如 1 分钟后填 60000）。禁止自己计算 ISO 时间点，交给服务端用 delayMs 基于当前时间计算，避免时区错误。
- 用户说“在某个具体时间点/某天几点”（绝对时间、一次性）=> 用 \`cron_job\` + \`type=at\` + \`at\`（ISO 8601，必须带 +08:00 时区偏移，例如 "2026-04-27T10:30:00+08:00"）。
- 用户说“每 X 分钟/每小时/每天”“定期/循环/一直”（重复执行）=> 用 \`cron_job\` + \`type=every\`（每次执行），\`everyMs\`=X 换算成毫秒
...
当用户请求“在未来某个时间点执行某个动作”（例如"1 分钟后给我发一个笑话到邮箱”）时，本轮对话只需要使用 \`cron_job\` 设置/更新定时任务，不要在当前轮直接完成这个动作本身：不要直接调用 \`send_mail\` 给他发邮件，也不要在当前轮就真正“执行”指令，只需把要执行的动作写进 \`instruction\` 里，交给将来的定时任务去跑。
...
注意：像"\`1 分钟后提醒我喝水\`"，时间相关信息用于计算下一次执行时间，而 \`instruction\` 应该是“提醒我喝水”；本轮不需要立刻提醒。`,
      ),
    ...
    throw new Error(
      `达到最大迭代次数限制 (${this.maxIterations} 次)，可能是工具调用死循环。请检查工具定义或调整 maxIterations 配置。`,
    );
  }
...
定时任务类型选择规则（非常重要）：
- "X 分钟/小时/天后”“到点提醒”（相对时间、一次性）=> \`cron_job.type=at\` + \`delayMs\`（延迟毫秒数，禁止自算 ISO，避免时区错误）
- “在某个具体时间点”（绝对时间、一次性）=> \`cron_job.type=at\` + \`at\`（ISO 8601，必须带 +08:00 时区偏移）
- “每 X 分钟/每小时/每天”“定期/循环/一直”（重复执行）=> \`cron_job.type=every\`（每次执行），\`everyMs\`=毫秒
