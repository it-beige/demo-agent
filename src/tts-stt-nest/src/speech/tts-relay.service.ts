...
// 单个会话:维护前端 WS + 对应的腾讯云 TTS WS,以及待发文本缓冲
type ClientSession = {
  sessionId: string;
  clientWs: WebSocket;
  tencentWs?: WebSocket;
  ready: boolean; // 腾讯云 push {ready:1} 后才能发文本
  pendingChunks: string[]; // ready 之前到达的 chunk 先缓冲
  closed: boolean;
};
...
export class TtsRelayService implements OnModuleDestroy {
    ...
    switch (event.type) {
      ...
      case 'chunk': {
        ...
        if (
          ...
          // ready 之前到的 chunk 暂存,等 ready=1 后再 flush
  ...
  // 建立到腾讯云 TTS 的 WebSocket(若已存在则复用)
