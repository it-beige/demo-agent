...
- 若已检索轮数已达到或超过最大检索轮数，必须 nextAction=generate。`
  ...
  const modelOutput = model.withStructuredOutput(NextStepSchema)
  const { nextAction, reason } = await modelOutput.invoke(prompt)
...
内容: ${item.content}`,
  ...
  process.stdout.write('\n【AI 回答（流式）】\n')
  let generation = ''
  const stream = await model.stream(`你是一个专业的《天龙八部》小说助手。基于小说内容回答问题，用准确、详细的语言。
